...
class data_loader(object):
    ...
    def __init__(self, path, paired_id):
        self.data = read_pkl_file("./../../data/OCL_test_data.pkl")
        self.cross = read_pkl_file("./../../data/OCL_crossed_data.pkl")
        self.cross = self.cross[paired_id[0]][paired_id[1]]
        self.path = path
    ...
    def get_next(self, attr):
        ...
        img1 = None
        if self.iteration_cnt % 5 == 0 and len(self.cross) > 0:
            rand_cross = random.choice(self.cross)
            #print(rand_cross)
            img1 = {"image_id":rand_cross["data_idx"], "object":self.data[rand_cross['data_idx']]['objects'][rand_cross["obj_idx"]]}
        while img1 == None:
            rand_img = random.choice(self.attr2item_index[attr])
