...
def load_pretrained_model_OneVision(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None,prompt_type="no" ,output_judge= False, tuning_type = "no", task_constraint="no", task_type="cand_chain",few_shot_num=1, **kwargs):
            ...
            if "mixtral" in model_name.lower():
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    ...
                    if overwrite_config is not None:
                        ...
                        llava_cfg = LlavaQwenConfig.from_pretrained(model_path)
                        llava_cfg.prompt_type = prompt_type
                        llava_cfg.output_judge = output_judge
                        llava_cfg.tuning_type = tuning_type
                        llava_cfg.task_constraint = task_constraint
                        llava_cfg.task_type = task_type
                        llava_cfg.few_shot_num = few_shot_num
                        model = LlavaQwenForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, attn_implementation=attn_implementation, config=llava_cfg, **kwargs)
    ...
    model.load_tokenizer_and_imageprocessor(tokenizer,image_processor)
